...
class FakeService:
    def __init__(self, *, host: str = '127.0.0.1', port: int = 19001) -> None:
        self.closes = 0
        self.roi_manager = SimpleNamespace(config=SimpleNamespace(bind=SimpleNamespace(host=host, port=port)))
...
class ServeCommandTest(SimpleTestCase):
    ...
    def patches(self):
        return (
            ...
            patch.object(serve_coordexp_refinement, 'Organization', self.organization_model),
            patch.object(serve_coordexp_refinement, 'ProductionRuntimeFactory', FakeFactory),
        )

    def test_serves_exact_ipv4_loopback_without_reloader_then_cleans_up(self) -> None:
        ...
        self.assertEqual(FakeFactory.instances[0].kwargs['repo_root'], Path('/data/CoordExp'))
        self.assertEqual(
            FakeFactory.instances[0].kwargs['roi_launch_config_path'],
            Path('/operator/roi-launch.json'),
        )

    def test_serves_exact_ipv6_loopback_without_reloader_then_cleans_up(self) -> None:
        FakeFactory.service = FakeService(host='::1')
        command = serve_coordexp_refinement.Command()
        user_patch, org_patch, factory_patch = self.patches()
        with (
            user_patch,
    ...
    def test_impossible_runtime_bind_fails_closed_and_cleans_up(self) -> None:
        FakeFactory.service = FakeService(host='0.0.0.0')
        command = serve_coordexp_refinement.Command()
        user_patch, org_patch, factory_patch = self.patches()
        with (
            user_patch,
    ...
    def test_worker_startup_failure_never_invokes_runserver(self) -> None:
        ...
        with (
            ...
            with self.assertRaisesRegex(CommandError, 'train worker initial recovery failed'):
                command.handle(**self.options())

        runserver.assert_not_called()
        self.assertEqual(FakeFactory.service.closes, 0)

    def test_independent_host_override_is_rejected_before_factory_start(self) -> None:
        command = serve_coordexp_refinement.Command()
    ...
    def test_independent_port_override_is_rejected(self) -> None:
        command = serve_coordexp_refinement.Command()
